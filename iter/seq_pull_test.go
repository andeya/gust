...
func TestIterator_Seq(t *testing.T) {
	...
	filtered := FromSlice([]int{1, 2, 3, 4, 5}).Filter(func(x int) bool { return x%2 == 0 })
	result = nil
...
func TestFromSeq(t *testing.T) {
	...
	gustIter, deferStop := FromSeq(goSeq)
	defer deferStop()
	...
	gustIter, deferStop = FromSeq(customSeq)
	defer deferStop()
	result = nil
	for {
		opt := gustIter.Next()
	...
	gustIter, deferStop = FromSeq(emptySeq)
	defer deferStop()
	assert.True(t, gustIter.Next().IsNone())
	...
	gustIter, deferStop = FromSeq(goSeq2)
	defer deferStop()
...
func TestSeq_RoundTrip(t *testing.T) {
	...
	originalForSeq := FromSlice([]int{1, 2, 3, 4, 5})
	originalForExpected := FromSlice([]int{1, 2, 3, 4, 5})

	seq := originalForSeq.Seq()
	converted, deferStop := FromSeq(seq)
	defer deferStop()
...
func TestSeq_WithMap(t *testing.T) {
	...
	iter := FromSlice([]int{1, 2, 3}).Map(func(x int) int { return x * 2 })
	var result []int
	for v := range iter.Seq() {
		result = append(result, v)
	}
...
func TestFromSeq_WithCustomSequence(t *testing.T) {
	...
	it2, deferStop2 := FromSeq(sliceSeq)
	defer deferStop2()
	result2 := it2.Collect()
...
func TestSeq2(t *testing.T) {
	...
	enumerated := Enumerate(FromSlice([]string{"x", "y", "z"}))
	...
	// Test with empty iterator
	empty := Zip(Empty[int](), Empty[string]())
	var emptyResult []gust.Pair[int, string]
	for k, v := range Seq2(empty) {
	...
	// Test early termination
	iter1 = FromSlice([]int{1, 2, 3, 4, 5})
	iter2 = FromSlice([]string{"a", "b", "c", "d", "e"})
	zipped = Zip(iter1, iter2)
...
func TestFromSeq2(t *testing.T) {
	...
	gustIter, deferStop := FromSeq2(seq2)
	defer deferStop()
	...
	gustIter, deferStop = FromSeq2(emptySeq2)
	defer deferStop()
	assert.True(t, gustIter.Next().IsNone())
	...
	var customIter Iterator[gust.Pair[int, string]]
	customIter, deferStop = FromSeq2(customSeq2)
	defer deferStop()
	...
	var chainIter Iterator[gust.Pair[int, int]]
	chainIter, deferStop = FromSeq2(seq2Chain)
	defer deferStop()
...
func TestSeq2_RoundTrip(t *testing.T) {
	...
	iter1ForSeq := FromSlice([]int{1, 2, 3})
	iter2ForSeq := FromSlice([]string{"a", "b", "c"})
	originalForSeq := Zip(iter1ForSeq, iter2ForSeq)

	iter1ForExpected := FromSlice([]int{1, 2, 3})
	iter2ForExpected := FromSlice([]string{"a", "b", "c"})
	originalForExpected := Zip(iter1ForExpected, iter2ForExpected)

	seq2 := Seq2(originalForSeq)
	converted, deferStop := FromSeq2(seq2)
	defer deferStop()
...
func TestSeq2_WithGoStandardLibrary(t *testing.T) {
	// Test that Seq2 works with Go's standard library iter.Pull2
	iter1 := FromSlice([]int{1, 2, 3})
	iter2 := FromSlice([]string{"a", "b", "c"})
	zipped := Zip(iter1, iter2)
	seq2 := Seq2(zipped)
...
func TestPull2(t *testing.T) {
	...
	// Test with Enumerate
	enumerated := Enumerate(FromSlice([]string{"x", "y", "z"}))
	nextEnum, stopEnum := Pull2(enumerated)
	defer stopEnum()
...
func TestFromPull(t *testing.T) {
	...
	gustIter, _ = FromPull(customNext, customStop)
	result = nil
	for {
		opt := gustIter.Next()
		if opt.IsNone() {
	...
	gustIter, _ = FromPull(emptyNext, emptyStop)
	assert.True(t, gustIter.Next().IsNone())
	...
	gustIter, _ = FromPull(next2, stop2)
	filtered := gustIter.Filter(func(x int) bool { return x > 2 })
	result = filtered.Collect()
	assert.Equal(t, []int{3, 4}, result)
}
...
func TestFromPull2(t *testing.T) {
	...
	gustIter, deferStop = FromPull2(emptyNext, emptyStop)
	defer deferStop()
	assert.True(t, gustIter.Next().IsNone())
	...
	var chainIter Iterator[gust.Pair[int, int]]
	chainIter, deferStop = FromPull2(next2, stop2)
	defer deferStop()
	// Filter pairs where value > 3
	// Sequence: (0,0), (1,2), (2,4), (3,6), (4,8)
	// Filter p.B > 3: (2,4), (3,6), (4,8) = 3 items
