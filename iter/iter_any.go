...
func (iter *AnyIter[T]) Fold(init any, f func(any, T) any) any {
	var accum = init
	for {
		x := iter.Next()
		if x.IsNone() {
...
func (iter *AnyIter[T]) TryFold(init any, f func(any, T) gust.Result[any]) gust.Result[any] {
...
func (iter *AnyIter[T]) Last() gust.Option[T] {
...
func (iter *AnyIter[T]) AdvanceBy(n uint) gust.Result[struct{}] {
	for i := uint(0); i < n; i++ {
		if iter.Next().IsNone() {
...
func (iter *AnyIter[T]) Nth(n uint) gust.Option[T] {
...
func (iter *AnyIter[T]) ForEach(f func(T)) {
...
func (iter *AnyIter[T]) Reduce(f func(accum T, item T) T) gust.Option[T] {
	var first = iter.Next()
	if first.IsNone() {
		return first
	}
...
func (iter *AnyIter[T]) Find(predicate func(T) bool) gust.Option[T] {
	var check = func(f func(T) bool) func(any, T) gust.Result[any] {
		return func(_ any, x T) gust.Result[any] {
			if f(x) {
...
func (iter *AnyIter[T]) FindMap(f func(T) gust.Option[any]) gust.Option[any] {
...
// assert.True(t, IsErr());
func (iter *AnyIter[T]) TryFind(predicate func(T) gust.Result[bool]) gust.Result[gust.Option[T]] {
