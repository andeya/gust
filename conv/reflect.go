...
// EnsurePointerInitialized initializes nil pointers with zero values recursively.
// It traverses through pointer chains and interfaces, initializing any nil pointers
// it encounters until it reaches a non-pointer type.
//
// Returns result.VoidResult indicating success or failure. Failure occurs when
// the value cannot be set (e.g., unexported fields or unaddressable values).
//
// NOTE: For interface values containing nil pointers, the function will attempt
// to initialize the pointer, but if the interface value itself is not settable,
...
//	res := conv.EnsurePointerInitialized(v)
//	if res.IsOk() {
//	    // x is now initialized to *int with value 0
//	}
//	```
func EnsurePointerInitialized(v reflect.Value) result.VoidResult {
	for {
		kind := v.Kind()
		if kind == reflect.Interface {
			elem := v.Elem()
			if !elem.IsValid() {
				// Interface contains nil, cannot proceed
				return result.OkVoid()
			}
			// If interface contains a pointer, we need to handle it through the interface
			if elem.Kind() == reflect.Ptr && elem.IsNil() {
				// Try to set a new pointer through the interface
				if !v.CanSet() {
					return result.FmtErrVoid("cannot set interface value: value is not settable")
				}
		...
		if !v.CanSet() {
			return result.FmtErrVoid("cannot set value: value is not settable")
		}
