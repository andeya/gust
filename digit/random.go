...
const (
	...
	// MAX: base62=ZZZZZZ, base36=zzzzzz, unix=56800235583, time=3769-12-05 11:13:03 +0800 CST
	timestampSecondLength = 6
	// timestampSecondMax is the maximum UNIX timestamp in seconds that can be encoded
	timestampSecondMax = 56800235583
)
...
// caseSensitive: if true, uses base36 (0-9, a-z); if false, uses base62 (0-9, a-z, A-Z)
...
// unixTs: UNIX timestamp in seconds, range [0, 56800235583] (1970-01-01 to 3769-12-05)
func (r *Random) RandomStringWithTime(length int, unixTs int64) result.Result[string] {
	...
	if unixTs < 0 || unixTs > timestampSecondMax {
		return result.FmtErr[string]("unixTs is out of range [0,%d]", timestampSecondMax)
	}
	return r.RandomString(length - timestampSecondLength).AndThen(func(randomPart string) result.Result[string] {
		return r.formatTimestamp(unixTs).Map(func(timestampPart string) string {
...
func (r *Random) RandomStringWithCurrentTime(length int) result.Result[string] {
	return r.RandomStringWithTime(length, time.Now().Unix())
}

// formatTimestamp formats a UNIX timestamp to a fixed-length string using the specified base
// The result is always timestampSecondLength characters long, padded with '0' on the left if necessary
func (r *Random) formatTimestamp(unixTs int64) result.Result[string] {
	formatted := FormatInt(unixTs, r.base)
	if len(formatted) > timestampSecondLength {
		// Timestamp exceeds maximum encodable length - return error instead of truncating
		return result.FmtErr[string]("timestamp %d exceeds maximum encodable length %d for base %d", unixTs, timestampSecondLength, r.base)
	}
	if len(formatted) < timestampSecondLength {
...
// ParseTime parses UNIX timestamp (in seconds) from the suffix of stringWithTime
// The timestamp is expected to be encoded in the last 6 characters of the string
func (r *Random) ParseTime(stringWithTime string) result.Result[int64] {
	...
	return ParseInt(stringWithTime[length-timestampSecondLength:], r.base, 64)
}
