...
func TestOption_AndThen_1(t *testing.T) {
	...
	assert.Equal(t, gust.Some("4"), option.AndThen(gust.Some[uint32](2), sqThenToString))
	assert.Equal(t, gust.None[string](), option.AndThen(gust.Some[uint32](1000000), sqThenToString))
	assert.Equal(t, gust.None[string](), option.AndThen(gust.None[uint32](), sqThenToString))
}
...
func TestOption_Unzip(t *testing.T) {
	...
	assert.Equal(t, option.Unzip(x), gust.Pair[gust.Option[int], gust.Option[string]]{A: gust.Some[int](1), B: gust.Some[string]("hi")})
	assert.Equal(t, option.Unzip(y), gust.Pair[gust.Option[int], gust.Option[string]]{A: gust.None[int](), B: gust.None[string]()})
}
...
func TestOption_Zip(t *testing.T) {
	...
	assert.Equal(t, gust.Some(gust.Pair[byte, string]{1, "hi"}), option.Zip(x, y))
	assert.Equal(t, gust.None[gust.Pair[byte, byte]](), option.Zip(x, z))
}
...
func TestOption_ZipWith(t *testing.T) {
	...
	assert.Equal(t, option.ZipWith(x, y, newPoint), gust.Some(Point{x: 17.5, y: 42.7}))
	assert.Equal(t, option.ZipWith(x, gust.None[float64](), newPoint), gust.None[Point]())
}
